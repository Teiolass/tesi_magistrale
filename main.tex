...
$$ O = \text{SoftMax} (Q K^\top/\sqrt{d_h})V $$  <self-attn>
...
\item The MLP layers has been replaced with a Swish Gated Linear Unit (SwiGLU). The function Swish
  ...
  $W_\text{gate}$, $W_\text{up}$ and $W_\text{down}$, of size $d_h\times d_\text{MLP}$ for the first two, and $d_\text{MLP}
 \times d_h$ for the last one. Let's denote the component-wise multiplication of two vectors with
  the symbol $\odot$. Then the layer is defined as
...
probability distribution over the next visit given the preceeding ones. In other words $\mu_\text{next} (v
...
A predictor model $h(\cdot, \theta)$ parametrized by a vector of parameters $\theta \in \R^d$, for some
...
codes $\codes$. This tree is called ontology and gives some informations over the codes in $\codes$
